...
class Agent(embodied.jax.Agent):
  ...
  def policy(self, carry, obs, mode='train'): # 这个函数的目的是根据输入的obs和carry，输出act和out
    (enc_carry, dyn_carry, dec_carry, prevact, curiosity_state) = carry
    ...
    carry = (enc_carry, dyn_carry, dec_carry, act, curiosity_state)
    if self.config.replay_context:
  ...
  def train(self, carry, data):
    ...
    # Reconstruct full carry with prevact and curiosity_state
    carry = (*carry, {k: data[k][:, -1] for k in self.act_space}, curiosity_state)
  ...
  def report(self, carry, data):
    ...
    carry, obs, prevact, _, curiosity_state = self._apply_replay_context(carry, data)
    (enc_carry, dyn_carry, dec_carry) = carry
    B, T = obs['is_first'].shape
    RB = min(6, B)
    metrics = {}
    ...
    carry = (*new_carry, {k: data[k][:, -1] for k in self.act_space}, curiosity_state)
  ...
  def _apply_replay_context(self, carry, data):
    ...
    if not self.config.replay_context:
      return carry, obs, prevact, stepid, curiosity_state
...
class CuriosityTrigger:
  """Stateless curiosity trigger with dynamic threshold.

  State is passed as a dictionary with 'mean' and 'var' keys,
  making it compatible with JAX's functional programming model.
  """
  ...
  def initial_state(self):
    """Return initial curiosity state."""
    return {'mean': 0.0, 'var': 1.0}

  def update(self, state, entropy):
    """Update curiosity state using exponential moving average.

    Args:
      state: Dict with 'mean' and 'var' keys
      entropy: Current entropy value

    Returns:
      Updated state dictionary
    """
    delta = entropy - state['mean']
    new_mean = state['mean'] + self.alpha * delta
    new_var = (1 - self.alpha) * state['var'] + self.alpha * (delta ** 2)
    return {'mean': new_mean, 'var': new_var}

  def should_explore(self, state, entropy):
    """Check if current entropy exceeds dynamic threshold.
    ...
    Returns:
      Tuple of (should_explore, threshold)
    """
    mean, std = state['mean'], jnp.sqrt(state['var'])
